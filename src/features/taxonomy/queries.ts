...
/// Přeloží slugy profesí na jejich názvy pro zobrazení doporučení (guide T020).
/// Vrací mapu slug → profese; neznámé slugy v mapě chybí (volající si doplní
/// fallback). Zahrnuje i archivované profese, aby už rozhodnuté doporučení
/// nezmizelo kvůli pozdější změně číselníku.
export async function getProfessionsBySlugs(
  ...
  return new Map(rows.map((p) => [p.slug, { slug: p.slug, name: p.name }]));
}
